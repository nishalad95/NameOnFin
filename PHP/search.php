...
$sql = "(SELECT `name`, `id`, `key_` FROM `schools` WHERE `name` LIKE '%$term%' OR `email` LIKE '%$term%' LIMIT 20) 
		UNION 
		(SELECT `name`, `id`, `key_` FROM `fin_names_near` WHERE `name` LIKE '%$term%' OR `email` LIKE '%$term%'  LIMIT 100)
		UNION
		(SELECT `name`, `id`, `key_` FROM `fin_names_off` WHERE `name` LIKE '%$term%' OR `email` LIKE '%$term%' LIMIT 100)
		UNION
...
/************ New search function using coordinates to replace above *************/
/*
// define variables and set to empty values
$term = "";
...
$sql = "(SELECT `name`, `id`, `key_`, `coord` FROM `schools` WHERE `name` = '$term')";
